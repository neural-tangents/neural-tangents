  ...
  if hasattr(k, '_asdict'):
    ...
    for key, value in k_dict.items():
      if key == 'var1':
        ...
        # NOTE: Currently we have to make these values concrete so that
...
def _merge_dicts(a, b):
  # TODO: Replace by {**a, **b} when Python 2 is depricated.
  merged = dict(a)
  merged.update(b)
  return merged
...
def _get_jit_or_pmap_broadcast():
  ...
  def jit_or_pmap_broadcast(f, device_count=-1):
    ...
    Raises:
      ...
      TODO: treat `np.ndarray`s in `kwargs` when JAX allows it. See
    ...
    def f_pmapped(x_or_kernel, *args, **kwargs):
      ...
      # TODO: treat `np.ndarray`s in `kwargs` when JAX allows it.
      # https://github.com/google/jax/issues/912
